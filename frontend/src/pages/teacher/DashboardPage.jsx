...
export default function DashboardPage() {
  ...
  useEffect(() => {
    api.get(`/parent/courses/${id}/progress`)
  ...
  const childrenProgress = data?.childrenProgress || [];

  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate(`/parent/courses`)} className="text-gray-400 hover:text-gray-600">←</button>
          <div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-4 py-3 font-medium text-gray-600">Barn</th>
                ...
                <tbody>
                  {childrenProgress.map(({ child, status, stars, aiSummary }) => (
                    <tr key={child.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-900">{child.name}</div>
                        <div className="text-xs text-gray-400">{child.username}</div>
                      </td>
